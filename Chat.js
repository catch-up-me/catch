...
const { BackArrow, VerifiedBadge, ThreeDots, ThreeDotsWhite, Search, Mute, AddContact, Block, Delete, ReplyArrow, DoubleCheck, Close, Download, Share, DeleteWhite } = window.SVGIcons;
...
function SentMessage({ id, content, time, isImage, imageSrc, swipedMessageId, swipeOffset, onSwipe, onResetSwipe, onImageClick }) {
  ...
  return (
    ...
    >
      <div 
        className="absolute right-full mr-3 top-1/2 -translate-y-1/2 transition-opacity duration-200"
        style={{ opacity: displayOffset > 30 ? 1 : 0, zIndex: 0 }}
      >
      ...
      <div 
        ...
        style={{ 
          ...
          padding: isImage ? '8px 6px 6px 6px' : '8px',
          zIndex: 1
        }}
      >
        <div className="absolute" style={{ 
          backgroundColor: '#d9fdd3', right: '-8px', top: '0', width: '20px', height: '20px',
          borderRadius: '0 0 0 20px', clipPath: 'polygon(0 0, 100% 0, 0 100%)'
        }}></div>
        <div className="absolute" style={{ 
          backgroundColor: '#d9fdd3', right: '-8px', bottom: '0', width: '20px', height: '20px',
          borderRadius: '20px 0 0 0', clipPath: 'polygon(0 0, 100% 100%, 0 100%)'
        }}></div>
        ...
        {isImage ? (
          ...
          <>
            <div className="text-black text-base mb-0.5">{content}</div>
            <div className="text-xs text-right flex items-center justify-end gap-1" style={{ color: '#667781', marginLeft: '40px' }}>
              {time}
              <DoubleCheck />
            </div>
          </>
    ...
    >
      ...
      >
        <div className="absolute bg-white" style={{ 
          ...
          borderRadius: '0 0 20px 0', clipPath: 'polygon(0 0, 100% 0, 100% 100%)'
        }}></div>
        <div className="absolute bg-white" style={{ 
          left: '-8px', bottom: '0', width: '20px', height: '20px',
          borderRadius: '0 20px 0 0', clipPath: 'polygon(0 100%, 100% 100%, 100% 0)'
        }}></div>
        ...
        {isImage ? (
          ...
          <>
            <div className="text-black text-base mb-0.5">{content}</div>
            <div className="text-xs text-right" style={{ color: '#667781', marginLeft: '60px' }}>{time}</div>
          </>
        )}
      </div>
    </div>
...
// Image Viewer Component with Swipe
    ...
    >
      ...
      >
        ...
        <div className="relative">
          <button className="p-2 hover:bg-gray-800 rounded-full transition-colors" onClick={() => setShowMenu(!showMenu)}>
            <ThreeDotsWhite />
          </button>
          ...
          {showMenu && (
            <>
              ...
              <div className="absolute right-0 top-full mt-2 w-48 rounded-lg overflow-hidden z-50" style={{ backgroundColor: '#1a1a1a', boxShadow: '0 10px 40px rgba(0, 0, 0, 0.5)' }}>
      ...
      >
        ...
        >
          {images.map((img, idx) => (
            <div key={idx} className="flex-shrink-0 w-full h-full flex items-center justify-center" style={{ minWidth: '100%' }}>
              <img src={img.src} alt={img.caption} className="max-w-full max-h-full object-contain" style={{ pointerEvents: 'none' }} />
            </div>
          ))}
        </div>
      </div>
...
function Chat({ conversation, onClose }) {
  ...
  const images = [
    { src: 'https://i.ibb.co/C5b875C6/Screenshot-20250904-050841.jpg', sender: 'Chizaram', time: 'Tue 10:42 AM', caption: 'Photo from Chizaram' },
    { src: 'https://i.ibb.co/C5b875C6/Screenshot-20250904-050841.jpg', sender: 'Mykee', time: 'Wed 08:52 PM', caption: 'Photo from Mykee' }
  ];
  ...
  return (
    <div className={`fixed inset-0 bg-white z-50 flex flex-col transition-transform duration-500 ease-in-out`} style={{ transform: isClosing ? 'translateX(100%)' : 'translateX(0)' }}>
      <header className="relative flex items-center justify-between px-4 py-3 bg-white shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div className={`w-10 h-10 rounded-full ${conversation.avatarColor} flex items-center justify-center overflow-hidden`}>
            {conversation.isText ? (
              ...
              <img src={conversation.avatarImage} alt={conversation.name} className="w-full h-full object-cover" onContextMenu={(e) => e.preventDefault()} draggable="false" />
            )}
          </div>
        ...
        <div className="relative">
          <button className="p-2 hover:bg-gray-100 rounded-full transition-colors" onClick={() => setShowMenu(!showMenu)}>
            <ThreeDots />
          </button>
      ...
      <main className="flex-1 bg-white overflow-y-auto" style={{ 
        ...
        <div className="max-w-3xl mx-auto px-5 py-5">
          <div className="text-center my-3">
            <span className="inline-block px-3 py-1 rounded-full text-xs font-medium" style={{ backgroundColor: '#d9d9d9', color: '#666' }}>May 30</span>
          </div>

          <ReceivedMessage id="msg1" content="Mykee Blogger" time="Tue 10:40 AM" swipedMessageId={swipedMessageId} swipeOffset={swipeOffset} onResetSwipe={resetSwipe} />
          <ReceivedMessage id="msg2" isImage={true} imageSrc="https://i.ibb.co/C5b875C6/Screenshot-20250904-050841.jpg" time="Tue 10:42 AM" swipedMessageId={swipedMessageId} swipeOffset={swipeOffset} onResetSwipe={resetSwipe} onImageClick={() => { setCurrentImageIndex(0); setSelectedImage(images[0]); }} />
          <SentMessage id="msg-sent-1" isImage={true} imageSrc="https://i.ibb.co/C5b875C6/Screenshot-20250904-050841.jpg" time="Wed 08:52 PM" swipedMessageId={swipedMessageId} swipeOffset={swipeOffset} onResetSwipe={resetSwipe} onImageClick={() => { setCurrentImageIndex(1); setSelectedImage(images[1]); }} />
          <SentMessage id="msg-sent-2" content="Who be this" time="Thu 10:56 AM" swipedMessageId={swipedMessageId} swipeOffset={swipeOffset} onResetSwipe={resetSwipe} />
          <ReceivedMessage id="msg3" content="Messiah" time="Fri 10:58 AM" swipedMessageId={swipedMessageId} swipeOffset={swipeOffset} onResetSwipe={resetSwipe} />
        </div>
      </main>

      {selectedImage && <ImageViewer images={images} currentIndex={currentImageIndex} onClose={() => setSelectedImage(null)} onIndexChange={setCurrentImageIndex} />}
    </div>
  );
}
