...
function MessagingHeader() {
  ...
  const conversations = [
    { id: 1, name: 'Chizaram', message: "Yo! Chizaram's in", avatar: 'C', avatarColor: 'bg-green-500', day: 'Wed', unread: 1 },
    { id: 2, name: 'VaVia', message: 'Hey, how are you doing?', avatar: 'V', avatarColor: 'bg-purple-500', day: 'Tue', read: true }
  ];

  const openConversation = (conv) => setOpenChat(conv);
  ...
  return (
    <>
      <div className={`w-full max-w-md mx-auto bg-white relative transition-transform duration-300 ${openChat ? 'animate-push-left' : ''}`}>
        ...
        {/* FAB */}
        <button className={`fixed right-6 bottom-6 w-14 h-14 rounded-full text-white text-3xl font-light flex items-center justify-center shadow-lg hover:shadow-xl transition-all active:scale-90 z-40 ${openChat ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
          style={{ backgroundColor: '#749cbf' }}>+</button>
        ...
        <div className="flex border-b border-gray-200">
          {tabs.map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              className={`flex-1 py-3 text-sm font-medium transition-colors relative ${activeTab === tab.id ? 'text-[#749cbf]' : 'text-gray-400 hover:text-gray-600'}`}>
              {tab.label}
              {activeTab === tab.id && <div className="absolute bottom-0 left-0 right-0 h-0.5" style={{ backgroundColor: '#749cbf' }} />}
            </button>
          ))}
        </div>
            ...
            {conversations.map((conv, idx) => (
              <div key={conv.id} onClick={() => openConversation(conv)}
                ...
                {idx < conversations.length - 1 && (
                  <div className="absolute bottom-0 h-px bg-gray-200" style={{ left: '4rem', right: 0 }} />
                )}
              </div>
            ))}
          </div>
        ) : activeTab === 'stories' ? (
          // This comes from story.js
          <StoryItem />
        ) : (
      ...
      {openChat && <Chat conversation={openChat} onClose={closeChat} />}
    </>
  );
}
...
style.textContent = `
  ...
  .shadow-xs { box-shadow: -14px 1px 14px 5px rgba(0, 0, 0, 0.05); }
`;
